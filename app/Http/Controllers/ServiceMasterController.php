...
/**
 * @property \App\Http\Controllers\CommonController CommonController
 */
...
//            ->where('engineer_master.AssignTo', Auth()->User()->id)
    ...
    public function fetchservices(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            $status = ['2', '4'];
            if(!empty($request->data['type']) && $request->data['type'] == 'COMPLETED') {
                $status = ['1','3','5','6','7','8','9'];
            }

            $data = DB::table('service_master')
                ->join('service_status', 'service_status.ServiceStatusId', '=', 'service_master.ServiceStatusId')
                ->whereIn('service_master.ServiceStatusId', $status)
                ->where('service_master.ServiceStatus', 1)
                ->where('EngineerId', $request->data['EngineerId'])
                ->get();
            if (count($data) > 0) {
    ...
    {
//        $request->serviceDetail = json_decode($request->serviceDetail, true);
//        $request->travellingDetail = json_decode($request->travellingDetail, true);
//        $request->serviceworkDetail = json_decode($request->serviceworkDetail, true);
    ...
    public function ServiceFinalComplete(Request $request)
    {
        ...
        $serviceDetailsInsertData = DB::table('service_details')
            ...
            ->update([
                ...
                'ToatalTA' => $request->serviceDetail[0]['ToatalTA'],
                'TotalDays' => $request->serviceDetail[0]['TotalDays'],
                'TotalTAExpenses' => $request->serviceDetail[0]['TotalTAExpenses'],
                'TotalExpense' => $request->serviceDetail[0]['TotalExpense'],
                'ExtraExpense' => $request->serviceDetail[0]['ExtraExpense'],
            ]);
        ...
        if (!empty($expenseDetail)) {
            $expense_detail_data = DB::table('expense_details')->insert($expenseDetail);
        }
    ...
    public function getServiceDetail(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            $service = DB::table('service_master')
                ...
                ->leftJoin('engineer_allowance', 'engineer_allowance.ServiceId', '=', 'service_details.ServiceId')
                ->where('service_master.ServiceId', $request->data['ServiceId'])
                ->first();

            $service->service_work_detail = DB::table('service_work_detail')
                ->where('service_work_detail.ServiceId', $service->ServiceId)
                ->get();
            $service->travelling_details = DB::table('travelling_details')
                ->where('travelling_details.ServiceId', $service->ServiceId)
                ->get();
            $service->expense_details = DB::table('expense_details')
                ->where('expense_details.ServiceId', $service->ServiceId)
                ->get();


            $dataToSend = [
                'service_no' => $service->TaskNumber,
                'service_location' => $service->ServiceLocation,
                'service_zone' => $service->RailwaysZone,
                'report_file' => $service->ReportFile,
                'total_ta' => $service->TotalTAExpenses,
                'travel_expense' => $service->TotalExpense,
                'extra_expense' => $service->ExtraExpense,
                'total_expense' => $service->TotalExpense + $service->ExtraExpense,
                'total_leaves' => $service->TotalLeaveDays,
                'status' => $service->ServiceStatusName,
            ];

            return $this->CommonController->successResponse($dataToSend, 'Data Fetch Successfully', 200);
        }
    }
